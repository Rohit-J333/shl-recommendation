...
- Top-**50** candidates (post hybrid scoring + domain boost) are passed to **Gemini 2.5 Flash** with a structured prompt: rerank by relevance, enforce skill-specific test priority (exact Java/SQL tests before generic cognitive), apply duration penalty, and ensure K/P balance for mixed roles.
- Domain/seniority-boosted candidates are injected at the **top** of the candidate list so they always fall within this 50-item window.
- Returns per-item explanations displayed in the UI.
- Retry logic handles Gemini free-tier 429 rate limits.

### Frozen Phase 3 Configuration _(final — submitted)_

| Parameter              | Value                                                        |
| ---------------------- | ------------------------------------------------------------ |
| Embedding model        | `all-MiniLM-L6-v2` (sentence-transformers, 384-dim)          |
| FAISS index type       | `IndexFlatIP` (cosine via L2-norm + inner product)           |
| Dense weight           | **0.60**                                                     |
| BM25 weight            | **0.40**                                                     |
| Candidate pool size    | **100** (top-100 from each of FAISS and BM25, merged)        |
| Domain boost injection | **Top** of candidate list (ensures visibility in LLM window) |
| LLM reranker window    | **50** candidates → selects top 10                           |
| LLM model              | `gemini-2.5-flash`                                           |
| Output size            | **10** recommendations per query                             |
...
| Phase 3: Domain boost fix + top-injection        | Corrected DOMAIN_BOOST URL slug patterns; injected boosted candidates at TOP of LLM window | **0.5767**     |

Key per-query results (final config):

| Query                                  | Recall@10           |
| -------------------------------------- | ------------------- |
| COO for China (cultural fit)           | **0.83**            |
| Java developer + collaboration         | **0.60**            |
| Mirchi Radio JD (media manager)        | **0.60** ← was 0.00 |
| Content Writer (English + SEO)         | **0.60**            |
| ICICI Bank Admin (0–2 yrs)             | **0.67**            |
| Marketing Manager (community/brand)    | **0.40** ← was 0.00 |
| Consultant (I/O psychology)            | **0.40** ← was 0.00 |
| Senior Data Analyst (SQL/Excel/Python) | **0.40**            |
| Sales role new graduates               | 0.33                |
| QA Engineer (Selenium/Java/SQL)        | 0.33                |

Queries previously scoring 0.0 now all have ≥ 0.40 recall after Phase 3 fixes: domain-specific assessments (e.g., `verify-verbal-next-generation`, `shl-verify-interactive-numerical-calculation`, `marketing-new`, `manager-8-0-jfa-4310`) were always present in the catalog but were being injected below position 100 in the candidate list, placing them outside the LLM reranker's top-50 window. Moving injected items to the top of the list resolved this.
...
**Limitations**: With the Phase 3 configuration, all previously zero-recall queries (media manager, I/O consultant, B2B marketing manager) now score ≥0.40. Remaining gaps in the lower-scoring queries (sales grads, QA engineer at 0.33) are due to large relevant sets (9–10 items) where the catalog's short descriptions do not carry enough signal to distinguish all relevant assessments. Future work: expand catalog descriptions via detail-page scraping, fine-tune a domain-specific embedding model, or use retrieval-augmented catalog enrichment.
